...
message_CoreNLP_gender = (
    "The gender annotator is only available for the Stanford CoreNLP package and the English language."
)

title_options_CoreNLP_quote = ["CoreNLP quote/speaker annotator"]
message_CoreNLP_quote = (
    "The quote/speaker annotator is only available for the Stanford CoreNLP package and the English language."
)
...
message_GIS_MWL = (
    "Locations known under different names (e.g., 'United States', 'US', 'USA'), can be all geocoded under a single name (e.g., 'United States'). The multiple name locations are listed in the csv file multi_name_locations.csv."
...
title_options_CoreNLP_Sentiment_Analysis_system_requirements = [
    "Stanford CoreNLP Sentiment Analysis system requirements"
]
...
message_IO_setup_date_options = (
    "Some of the algorithms in the NLP Suite (e.g., GIS models and network models) can build dynamic models (i.e., models that vary with tiime) when time/date is known.\n\n"
...
message_data_manager_merge4 = (
    "Please, click next the + sign next to File at the top of the GUI to select another csv file to merge."
)
...
message_data_manager_merge7 = (
    "Please, click next the + sign next to File at the top of the GUI to select another csv file to merge or click RUN."
)
...
def checkReminder(scriptName, title_options=None, message="", triggered_by_GUI_event=False, silent=False):
            ...
            if len(df1) != 0:
                ...
                if status == "Yes" or status == "ON":  # 'Yes' the old way of saving reminders
                    if not silent:
                        ...
                        displayReminder(
                            df, row_num, title, message, status, "\n\nDo you want to see this message again?", True
                        )
...
def resetReminder(scriptName, title):
    ...
    if title != "Open reminders":
        ...
        try:
            ...
            logger.info(
                'Reminders file error The reminders.csv file saved in the reminders subdirectory is ill formed. Most likely, it contains extra , in one of the three fields (Routine, Title, Message).\n\nPlease, let the NLP Suite development team know the problem so it can be fixed.\n\nIf any of the fields contain , the field content must be enclosed in "".'
            )
            return
        try:
            df1 = df.loc[(df["Routine"] == routine) & (df["Title"] == title)]
