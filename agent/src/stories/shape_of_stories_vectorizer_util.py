...
class Vectorizer:
        ...
        for _narrativeNum, narrativeFile in enumerate(self.narrative_file_paths):
            with codecs.open(narrativeFile, mode="r", encoding="utf-8", errors="ignore") as csv_file:
                ...
                # ============== Angel ================= added additional checks for merged file
                    ...
                    addedFiles.append(
                        (str(IO_csv_util.undressFilenameForCSVHyperlink(narrativeFile))[:-4] + ".csv", re_merged)
                    )  # save to regenerate later
    ...
    def parse_input_data(self):
        ...
        if os.path.isfile(self.data_folder):
            ...
            narrative_fps = [
                ...
                if os.path.isfile(os.path.join(self.data_folder, f)) and not f.startswith(r".")
            ]
    ...
    def vectorize(self):
        ...
        with codecs.open(self.narrative_file_paths[0], mode="r", encoding="utf-8", errors="ignore") as nF1:
            merged_df = pd.read_csv(nF1, encoding="utf-8", on_bad_lines="skip")
        # Add sentimentscores Filenames as a column
        merged_df["scoresFilename"] = [self.narrative_file_paths[0]] * merged_df.shape[0]
        if (
            "Document" not in merged_df.columns and "Document Name" in merged_df.columns
        ):  # sometimes "Document" sometimes "Document Name"
            ...
            for narrativeNum, narrativeFile in enumerate(self.narrative_file_paths, start=1):
                if narrativeNum > 1:
                    with codecs.open(narrativeFile, mode="r", encoding="utf-8", errors="ignore") as narrativeFile1:
                        dfff = pd.read_csv(narrativeFile1, encoding="utf-8", on_bad_lines="skip")
        ...
        for df in df_list:
            ...
            window = [
                float(row["Sentiment score"]) for i, row in df.iterrows() if i < self.window_size
            ]  # take the first [window_size] records and get the sentiment score
