...
class Visualizer:
    ...
    def visualize_clusters(
        self, numberOfSentimenFiles, grouped_vectors, method, method_short, cluster_file, modes=None
    ):
        titles = []
        file_names = []
        for i in range(len(grouped_vectors)):
            ...
            try:
                titles.append(
                    ...
                    + str(i + 1)
                    + " (N = "
                    + str(len(cluster_file[i]))
                    + "/"
                    + str(numberOfSentimenFiles)
                    + ")"
                )
...
def visualize_sentiment_arcs(
    sent_arcs, output_fig_path, plot_title="Scatter plot of Principal Component Analysis (PCA) of Sentiment Arcs"
):
    """
