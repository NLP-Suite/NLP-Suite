...
def get_words_minus_K_plus_K(
    ...
    left, mid, right = find_k_adjacent_tokens(
        words_, search_keyword, exact_word_match, minus_K_words_var, plus_K_words_var
    )
...
def search_in_all_sentences_oneDoc(
    file,
    create_subcorpus_var,
    corpus_to_copy,
    docText,
    ...
    for sentence_index, sentence in enumerate(sentences):
        ...
        for search_word in search_keywords_list:
            search_word_frequency = NGrams_CoOccurrences_util.get_search_word_from_text(
            ...
            if search_word_frequency == 0:
                ...
                if minus_K_var > 0 or plus_K_var > 0:
                    minus_plus_K_words = get_words_minus_K_plus_K(
                        sentence,
                        search_word,
                        exact_word_match,
                        minus_K_var,
...
def search_sentences_documents(
    inputFilename,
    inputDir,
    outputDir,
    configFileName,
    ...
    if create_subcorpus_var:
        ...
        subCorpusDir = IO_files_util.make_output_subdirectory(
            inputFilename, inputDir, inputDir, label="subcorpus_search", silent=False
        )
    ...
    for file in files:
        ...
        # search in sentence  -----------------------------------------------
            ...
            (
                ...
                all_found_csv_words_minusK_plusK_records_oneDoc,
                all_found_csv_sentences_records_oneDoc,
            ) = search_in_all_sentences_oneDoc(
                file,
                create_subcorpus_var,
                corpus_to_copy,
                docText,
    ...
    outputFilename_csv_word = IO_files_util.generate_output_file_name(
        inputFilename, inputDir, outputDir, ".csv", "search_word" + label
    )
    filesToOpen.append(outputFilename_csv_word)

    outputFilename_csv_word_NOT_found = IO_files_util.generate_output_file_name(
    ...
    # search in sentence
        ...
        header = [
            search_word_header,
            "Number of sentences",
            "Sentence ID of first occurrence",
        ...
        header_minusK_plusK = [
            "Minus K Value of Words (" + str(minus_K_var) + ")",
            search_word_header,
            "Plus K Value of Words (" + str(plus_K_var) + ")",
            "Number of sentences",
            "Sentence ID of first occurrence",
            "Relative position in document",
            "Frequency of occurrence",
