...
def get_csv_data(inputFilename, withHeader, inputFileData=""):
    ...
    if inputFileData:
        ...
        if b"\x00" in file_data:
            logger.info(
                f"Warning: null bytes found in {inputFilename} — removing them and overwriting the file in place."
            )
...
def get_csvfile_headers(csvFile, ask_Question=False, inputFileData=""):
    ...
    if csvFile != "" and answer:
        try:
            ...
            logger.info("Warning: Opening the csv file " + csvFile + " encountered an error.\n\n" + str(e))
            headers = ""
    ...
    try:
        ...
        if algorithm != "":
            msg = (
                "\n\nThe '"
                + algorithm
                + "' algorithm requires in input a csv file with a '"
                + columnHeader
                + "' column."
            )
        logger.info(
...
def extract_from_csv(inputFilename, outputDir, data_files, columns_to_export=None):
    ...
    outputFilename = IO_files_util.generate_output_file_name(
        inputFilename, "", outputDir, ".csv", "extract", "", "", "", "", False, True
    )
...
def export_csv_to_text(inputFilename, outputDir, column=None, column_list=None):
    ...
    if column is not None and len(column_list) != 0:
        logger.info(
            "Field(s) input error Cannot have field and field_list as filter at the same time.\n\nPlease, select one and try again."
        )
        return
