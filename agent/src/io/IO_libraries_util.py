...
def which_shell():
    ...
    if shell != "zsh":
        ...
        logger.info(f"{error_msg}: TIPS_NLP_Anaconda NLP environment pip.pdf")
    return shell
...
def import_nltk_resource(resource_path, resource):
    try:
        ...
        IO_user_interface_util.timed_alert(
            ...
            "The NLTK resource "
            + resource
            + " is missing. Starting to download nltk "
            + resource
...
def check_java_installation(script):
    ...
    if error_code != 0:
        if ("not recognized" in system_output) or (system_output == ""):
            message = (
                "A test for Java returned a non-zero error code "
                + str(error_code)
                + " and Java not recognized (You can check this in command line by typing Java -version)."
            )
        ...
        message = (
            message
            + script
            + " is a Java script that requires the freeware Java (by Oracle) installed on our machine.\n\n"
            ...
            for info in system_output.split(" "):
                if "-Bit" in info:  # find the information about bit
                    if info[:2] != "64":  # check if it's 64 bit
                        ...
                        logger.info(
                            "Java version Error You are not using JAVA 64-Bit version.\n\nThis will cause an error running Stanford CoreNLP: Could not create the Java Virtual Machine.\n\nPlease, configure your machine to use JAVA 64-Bit.\n\nPlease, read carefully the TIPS_NLP_Stanford CoreNLP memory issues.pdf.\n\nDo you want to open the TIPS file now?"
                        )
                        Java_errorFound = True
            ...
            if github_version != local_version:
                if not silent:
                    IO_user_interface_util.timed_alert(
                        ...
                        "Oops! Your local Stanford CoreNLP version is "
                        + local_version
                        + ".\n\nIt is behind the latest Stanford CoreNLP version available on GitHub ("
                        + github_version
...
def check_inputExternalProgramFile(calling_script, software_dir, programName, readingConfig=True, silent=False):
    ...
    if "Java" not in programName:  #!='Java (JDK)' Java (JDK)'
        ...
        if readingConfig:
            ...
            wrong_dir_msg = (
                "The selected software directory\n\n  "
                + software_dir
                + "\n\nis NOT the expected "
                + programName.upper()
                + " directory."
        ...
        if os.path.isdir(os.path.join(software_dir, tail)):
            unarchive_msg = (
                ...
                + " contains a "
                + tail
                + " directory inside. DO MAKE SURE THAT WHEN YOU UNARCHIVE THE "
                + programName.upper()
                + " ARCHIVE YOU DO NOT END UP WITH A "
                + programName.upper()
                + " DIRECTORY INSIDE A "
                + programName.upper()
                + " DIRECTORY."
        ...
        Mac_msg = (
            ...
            + programName.upper()
            + " double click on the downloaded .dmg file and drag the "
            + programName.upper()
            + " application in your Mac Applications directory."
        )
        ...
        if not os.path.isdir(software_dir):
            if "setup_external_software" in calling_script:
                reinstall_string = (
                    "\n\nPlease, reinstall "
                    + programName
                    + " by selecting the folder where the software is stored on your machine"
                )  # using the dropdown menu "Software INSTALL" and select the location where the software is stored on your machine'
            else:
    ...
    if programName == "Stanford CoreNLP":
        ...
        directory_content = (
            ...
            + "\n\nThe "
            + programName.upper()
            + " directory should contain, among other things, many files with 'ejml' and 'javax' in the filename."
        )
    ...
    if programName == "Gephi":
        if platform == "win32":
            ...
            directory_content = (
                ...
                + "\n\nThe "
                + programName.upper()
                + " directory should contain, among other things, the subdirectories 'gephi' and 'platform'"
            )
            message = directory_content + unarchive_msg + select_directory_msg
        if platform == "darwin":
    ...
    if programName == "MALLET":
        ...
        directory_content = (
            ...
            + "\n\nThe "
            + programName.upper()
            + " directory should contain, among other things, the subdirectories 'bin' and 'class'"
        )
        message = directory_content + unarchive_msg + select_directory_msg
    ...
    if programName == "WordNet":
        ...
        directory_content = (
            ...
            + "\n\nThe "
            + programName.upper()
            + " directory should contain, among other things, the subdirectories 'dict' and 'src'"
        )
        message = directory_content + unarchive_msg + select_directory_msg
...
def open_url(
    ...
    if reminder_title != "":
        reminders_util.checkReminder(scriptName, reminder_title, reminder_message, True)
...
def get_missing_external_software_list(calling_script, external_software_config_file, software_name, silent=False):
    ...
    for row in external_software_config_file[1:]:  # skip header line
        ...
        software_dir = row[1]
        index = index + 1
        ExternalProgramFile_result = check_inputExternalProgramFile(
            calling_script, software_dir, software_name, True, silent
        )
        if (
            external_software_config_file[index][1] == ""
            or not os.path.isdir(software_dir)
    ...
    with open(software_config, "w+", newline="") as csv_file:
        ...
        if len(missing_software_list) > 0:
            ...
            message = (
                ...
                + str(len(missing_software_list))
                + " other remaining missing software to download and/or install: "
...
def process_Mac_Applications(software_name, software_extension):
    if not check_program_Mac_Applications(software_name):
        ...
        installation_message = (
            software_name.upper()
            + " IS NOT INSTALLED ON YOUR MACHINE (the Path cell is blank in the config file NLP_setup_external_software_config.csv)."
            "\n\nIf you have already downloaded the software, and just not installed it, double click on the downloaded "
            + software_extension
            + " file."
        )
        ...
        installation_message = (
            ...
            + software_name
            + ', and use the dropdown menu "Software DOWNLOAD" to select '
            + software_name
...
def display_download_installation_messages(
    download_install,
    software_name,
    software_dir,
    software_url,
    ...
    if (platform == "win32") or (
        ...
        software_location_message = (
            "DO NOT "
            + software_location_label
            + " THE SOFTWARE DIRECTORY INSIDE THE NLP SUITE DIRECTORY OR IT MAY BE OVERWRITTEN IN CASE YOU NEED TO RE-INSTALL THE SUITE."
            + MALLET_message
            + WordNet_Chrome_message
        )
    else:
    ...
    if software_name == "":
        ...
        opening_message = (
            "The NLP Suite relies on several external programs that need to be downloaded and installed.\n\n"
    ...
    if "download" in download_install:
        ...
        if software_dir is None or software_dir == "":
            ...
            if software_name != "":
                ...
                after_download_message = (
                    "\n\nAfter download is completed, double click on the "
                    + software_extension
                    + " file to "
                    + executing_unzipping_label
                ...
                (
                    ...
                    + executing_unzipping_label
                    + "ing, and perhaps moving the "
                    + software_name.upper()
                    + " software directory, you will be asked whether you also want to install "
                    + software_name.upper()
                ...
                if platform == "darwin":
                    ...
                    if software_name == "Google Earth Pro":
                        ...
                        after_download_message = (
                            "\n\nAfter download is completed, double click on the "
                            + software_extension
                            + " file to install Google Earth Pro. The installation process will automatically place Google Earth Pro in the Applications directory."
                        )
        ...
        # DOWNLOAD check software_dir ------------------------------------------------------------------------------
            ...
            if software_name == "WordNet":
                download_message = (
                    "Once the WORDNET website opens up, you need to download the executable file WordNet-2.1.exe. After downloading, run the executable file by double-clicking on it and move the WordNet directory "
    ...
    # INSTALL messages -------------------------------------------------------------------------
        ...
        installation_directory_message = (
            ...
            + Mac_Applications_label
            + "where \n\n"
            + software_name.upper()
            + "\n\nis saved on your machine. "
            "\n\nPress Esc or Cancel to exit when the dialogue box opens if you do not want to install "
            + software_name.upper()
            + ".\n\n"
            + software_location_message
        )
