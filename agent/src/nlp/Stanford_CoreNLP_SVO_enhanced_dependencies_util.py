...
def verb_obj_obl(token, sent_data, v_obj_obl_json):  # check if one verb is a part of an LVC
    ...
    if (
        verb_lemma not in v_obj_obl_json.keys()
    ):  # the case when that verb has no list of preposition collocation stored in lib file
        return "", "", ""
...
def advcl_building(token, sent_data, ner, p_s, p_o, v_obj_obl_json, v_prep_json):
    ...
    for idx in advcl:
        ...
        second_result, second_negation_result = advcl_extraction(
            advcl_token, sent_data, s, o, v_obj_obl_json, v_prep_json
        )
...
def verb_root_svo_building(
    verb, sent_data, v_obj_obl_json, v_prep_json
):  # extract the subject and object of a single verb (as well as processing modifiers of that verb)
...
def verb_root(
    verb_list, conj_word, token, sent_data
):  # extract subject, object, and negation of a single verb or multiple verbs that are conjuncts
    v_prep_text = (
        GUI_IO_util.CoreNLP_enhanced_dependencies_libPath + os.sep + "verb_prep_json.txt"
    )  # json that help with extracting object that follows a preposition
    v_obj_obl_text = (
        GUI_IO_util.CoreNLP_enhanced_dependencies_libPath + os.sep + "verb_obj_obl_json.txt"
    )  # json that help with extracting certain light verb constructions as a whole verb
    ...
    for verb in verb_list:
        ...
        if len(negation_list) > 0:
            if negation_list[0] and conj_word == "or":
                negation = True

        if not s_set and s != "inferred_subject_passive":  # setting potential subject
            s_set = True
            s_share = s
        if not o_set and o != "":  # setting potential object
            o_set = True
            o_share = o
            o_share_idx = o_idx
        if s == "inferred_subject_passive":
...
def negation_detect(token, sent_data):  # detect if there's negation associated with a verb
    ...
    dep_list = [
        ...
        "mark",
        "aux",
    ]  # the common dep of tokens tokens in negation_tokens
...
def link_verb_LVC_extraction(token, gov_dict, sent_data):
    ...
    link_verb_LVC_text = (
        GUI_IO_util.CoreNLP_enhanced_dependencies_libPath + os.sep + "link_verb_LVC_json.txt"
    )  # json that help with extracting object that follows a preposition
    with open(link_verb_LVC_text) as link_verb_LVC_doc:
        link_verb_LVC_json = json.load(link_verb_LVC_doc)

    if (
        token["lemma"] in link_verb_LVC_json.keys()
    ):  # that token is the dependency ROOT (the syntactical head of other tokens in that LVC) of that LVC
...
def SVO_extraction(sent_data, entitymentions):  # returns columns of the final output
    ...
    for item in entitymentions:
        ...
        if item["ner"] is not None and item["ner"] in ["PERSON"]:
            # SIMON
            if item["text"] not in person_list:
        ...
        if item["ner"] is not None and item["ner"] in ["ORGANIZATION"]:
            # SIMON
            if item["text"] not in organization_list:
    ...
    for key in sent_data.keys():  # traverse each token in that sentence
        ...
        gov_dict = token[
            "govern_dict"
        ]  # the dictionary that contains information of the dep and index of tokens whose syntactical head is the corrent token
        if (
            ("VB" in token["pos"])
            and ("advcl" not in token["deprel"])
            and ("xcomp" not in token["deprel"])
            and (token["deprel"] != "dep")
            and (token["deprel"] != "acl")
        ):  # if the verb has not been processed and its dep is not a special dep that will be processed independently
            ...
            elif (token["deprel"] == "ROOT" or token["deprel"] == "parataxis") and (
                "NN" in token["pos"] or token["pos"] == "PRP"
            ):  # Subject -> Verb(be) -> predicative nominative
