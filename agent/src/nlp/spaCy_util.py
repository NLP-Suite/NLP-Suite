...
def spaCy_annotate(
    ...
    **kwargs,
    ...
    startTime = IO_user_interface_util.timed_alert(
        2000,
        "Analysis start",
        "Started running spaCy " + str(annotator_params[0]) + " annotator at",
        True,
        "",
        True,
        "",
    ...
    reminders_util.checkReminder(
        scriptName,
        reminders_util.title_options_spaCy_parameters,
        reminders_util.message_spaCy_parameters,
        True,
    )
...
def convertSpacyDoctoDf(spacy_doc, inputFilename, inputDir, tail, docID, annotator_params, language):
    ...
    if "sentiment" in str(annotator_params):
        ...
        for sent in spacy_doc.sents:
            ...
            out_df.at[c, "Sentiment label"] = (
                "positive"
                if out_df.at[c, "Sentiment score"] > 0
                else "negative"
                if out_df.at[c, "Sentiment score"] < 0
                else "neutral"
            )
