...
def k_sent(
    ...
    for i in range(1, max(conll["Document ID"]) + 1):
        ...
        outputFilename_rep_words = IO_files_util.generate_output_file_name(
            inputFilename,
            "",
            outputDir,
            ".csv",
        ...
        result_rep_words.extend(
            [
                sublist
                for sublist in result_rep_words_temp
        ...
        if df_rep_words.empty:
            ...
            outputFiles = charts_util.visualize_chart(
                chartPackage,
                dataTransformation,
                outputFilename_rep_words,
                outputDir,
                columns_to_be_plotted_xAxis,
                columns_to_be_plotted_yAxis,
                chart_title="Frequency Distribution of Repeated Words in First and Last K ("
                + str(Begin_K_sent_var)
                + "-"
                + str(End_K_sent_var)
                + ") Sentences",
    ...
    for i in range(1, max(conll["Document ID"]) + 1):
        ...
        temp_first = [
            ...
            word_count_first,
            noun_count_first,
            noun_count_first / word_count_first,
            verb_count_first,
            verb_count_first / word_count_first,
            adj_count_first,
            adj_count_first / word_count_first,
            pp_count_first,
            pp_count_first / word_count_first,
            i,
            DOC,
        ]
        ...
        temp_last = [
            ...
            word_count_last,
            noun_count_last,
            noun_count_last / word_count_last,
            verb_count_last,
            verb_count_last / word_count_last,
            adj_count_last,
            adj_count_last / word_count_last,
            pp_count_last,
            pp_count_last / word_count_last,
            i,
            DOC,
        ]
        ...
        if df.empty:
            ...
            outputFiles = charts_util.visualize_chart(
                ...
                columns_to_be_plotted_xAxis,
                columns_to_be_plotted_yAxis,
                chart_title="Frequency Distribution of Different Proportions in First and Last K ("
                + str(Begin_K_sent_var)
                + "-"
                + str(End_K_sent_var)
                + ") Sentences",
