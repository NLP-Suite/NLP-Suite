...
def parsers_annotators_visualization(
    configFilename,
    inputFilename,
    inputDir,
    outputDir,
    ...
    if ("NER" in str(annotator_params) and "NER" in outputFilename) or "parse" in str(annotator_params):
        reminders_util.checkReminder(scriptName, reminders_util.NER_frequencies, reminders_util.message_NER_frequencies)
    ...
    if "parse" in str(annotator_params):  # and 'CoNLL' in outputFilename) or ('depparse' in str(annotator_params)):
        ...
        reminders_util.checkReminder(
            scriptName, reminders_util.DepRel_frequencies, reminders_util.message_DepRel_frequencies, True
        )
        outputFiles = charts_util.visualize_chart(
            chartPackage,
            dataTransformation,
    ...
    if ("SVO" in str(annotator_params) and "SVO" in outputFilename) or (
        ...
        if not os.path.isdir(outputSVOUnFilterDir):
            outputSVOUnFilterDir = IO_files_util.make_output_subdirectory("", "", head, label="SVO_form", silent=True)
...
def visualize_html_file(inputFilename, inputDir, outputDir, configFileName, dictFilename, genderCol=None, wordCol=None):
    ...
    outputFiles = html_annotator_dictionary_util.dictionary_annotate(
        inputFilename,
        inputDir,
        outputDir,
        configFileName,
