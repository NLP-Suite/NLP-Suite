...
def check_CoNLL(filename, skipWarning=False):
    ...
    if not all(x in headers for x in ["ID", "Form", "Lemma"]):
        ...
        if numColumns != 13 and numColumns != 14:
            wrongFile = True
    if wrongFile and not skipWarning:
        logger.info(
            "The script expects in input a CoNLL table with the headers ID, Form, and Lemma and either 14 or 13 columns (with/without date field).\n\nThe selected file does not have these expected characteristics (number of columns = "
            + str(numColumns)
...
def CoNLL_record_division(list_csv_rows):
    ...
    try:
        ...
        if len(list_sentences) == 0:
            ...
            logger.info(
                "The sentence_division function in "
                + currentScript
...
def sentence_division(list_csv_rows, searchedCoNLLField):
    ...
    try:
        ...
        if len(list_sentences) == 0:
            currentScript = os.path.basename(__file__)
            raise Exception("sentence_division in " + currentScript + " failed")
            # mb.showinfo("Fatal error",
            #             "The sentence_division function in " + currentScript + " failed.\n\nPlease, check your data and/or the python scripts.\n\nIf the problem persists, please inform the script developers of the problem.\n\nProgram will exit.")
        return list_sentences
    except Exception:
    ...
    try:
        _list_sorted_ = [
            [
                ...
                i[2],
                i[3],
                find_full_postag(i[1], i[3]),
                i[6],
                find_full_deprel(i[1], i[6]),
                i[clause_position],
                find_full_clausalTag(i[1], i[clause_position]),
        ...
        try:
            ...
            raise Exception("CoNLL table ill formed") from None
...
def compute_sentence_table(CoNLL_table, output_path):
    ...
    )  # os.path.join(output_path,outputFilename))     #Time when compute sentence table finished, for future reference
...
def get_nouns_verbs_CoNLL(inputFilename, output_dir):
    ...
    for index, _row in conll_table.iterrows():
