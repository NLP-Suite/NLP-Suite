...
def Stanza_annotate(
    ...
    **kwargs,
    ...
    for key, value in kwargs.items():
        ...
        if key == "google_earth_var" and value:
    ...
    if len(language) == 1 and language[0] != "multilingual":
        ...
        startTime = IO_user_interface_util.timed_alert(
            2000,
            "Analysis start",
            "Started running Stanza " + str(annotator_params) + " annotator at",
            True,
            "",
            True,
            "",
    ...
    if "SVO" in annotator_params:
        ...
        outputFilename = IO_files_util.generate_output_file_name(
            inputFilename, inputDir, outputDir, ".csv", file_label + "_Stanza"
        )
    ...
    IO_user_interface_util.timed_alert(
        2000,
        "Analysis end",
        "Finished running Stanza " + str(annotator_params) + " annotator at",
        True,
        "",
        True,
        startTime,
...
def convertStanzaDoctoDf(stanza_doc, inputFilename, inputDir, tail, docID, annotator_params, language):
    ...
    if annotator_params == "sentiment":
        for i, sentence in enumerate(stanza_doc.sentences):
            ...
            out_df.at[i, "Sentiment label"] = (
                "positive" if sentence.sentiment > 1 else "negative" if sentence.sentiment < 1 else "neutral"
            )
        ...
        out_df = out_df.drop(["xpos", "start_char", "end_char", "multi_ner", "feats"], axis=1, errors="ignore")
...
def extractSVO(doc, docID, inputFilename, inputDir, tail, filename_embeds_date_var, NER_available):
    ...
    if filename_embeds_date_var:
        ...
        svo_df = pd.DataFrame(
            columns=["Subject (S)", "Verb (V)", "Object (O)", "Location", "Person", "Time", "Sentence ID", "Sentence"]
        )
    empty_verb_idx = []
    SVO_found = False
...
# input: Stanza DF
def excludePOS(df, postag=None):
    if postag is None:
        postag = {"NUM", "PUNCT"}
    for p in postag:
        df = df[not df["POS"].str.contains(p)]
    return df
...
if not os.path.exists(resources_path):
    raise ImportError(
        "Stanza does not seem to be installed on this machine. The file-path\n\n"
        + resources_path
        + "\n\ncould not be found.\n\nPlease run: pip install stanza"
    )
