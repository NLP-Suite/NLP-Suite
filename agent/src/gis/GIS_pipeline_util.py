...
def GIS_pipeline(
    ...
    icon_var_list,
    specific_icon_var_list,  # pushpin, red
    name_var_list,
    scale_var_list,
    color_var_list,
    ...
    config_filename.replace("_config.csv", "")
    ...
    startTime = IO_user_interface_util.timed_alert(
        2000,
        "Analysis start",
        "Started running GIS pipeline at",
        True,
        "",
        True,
        "",
    ...
    if inputIsCoNLL:
        outputCsvLocationsOnly = IO_files_util.generate_output_file_name(
            inputFilename,
            "",
            outputDir,
            ".csv",
        ...
        if not inputIsGeocoded and geocoder == "Nominatim":
            ...
            if datePresent:
                nom_df = (
                    pd.DataFrame(
                        locations,
                        columns=["Location", "Date", "NER", "Sentence", "Document"],
                    )
                ...
                nom_df = (
                    ...
                    if len(locations[0]) == 2
                        ...
                        pd.DataFrame(
                            locations,
                            columns=["Location", "NER", "Sentence", "Document"],
                        )
            ...
            if changed:
                ...
                tmp_df.to_csv(
                    inputFilename, index=False
                )  # TODO: drop a index column, which will produce error with producing KML (if selected).
    ...
    if not inputIsGeocoded:
        ...
        kmloutputFilename = GIS_geocode_util.process_geocoded_data_for_kml(
            locations,
            inputFilename,
            outputDir,
            locationColumnName,
    ...
    if len(locations) > 0 and inputIsCoNLL:
    ...
    if geocodedLocationsOutputFilename != "" and nRecordsFound > 0:
        ...
        if chartPackage != "No charts":
            ...
            outputFiles = charts_util.visualize_chart(
                ...
                outputDir,
                columns_to_be_plotted_xAxis=[],
                columns_to_be_plotted_yAxis=["Country from Geocoder"],
    ...
    if nRecordsFound > 0 and "folium" in mapping_package:
        ...
        folium_pinmap_outputFilename = IO_files_util.generate_output_file_name(
            inputFilename,
            inputDir,
            outputDir,
            ".html",
    ...
    if nRecordsFound > 0 and "Google Maps" in mapping_package:
        heatMapoutputFilename = IO_files_util.generate_output_file_name(
            inputFilename,
            "",
            outputDir,
            ".html",
    ...
    IO_user_interface_util.timed_alert(
        2000,
        "Analysis end",
        "Finished running GIS pipeline at",
        True,
        "",
        True,
        startTime,
