...
# ANALYSES -----------------------------------------------------------------------------------
    ...
    if all_analyses_var.get():
        ...
        if all_analyses.get() == '*' or all_analyses.get() == 'Clause analysis':
            # Clause analysis requires Clause Tag — only available from CoreNLP PCFG parser
            if source_package != 'CoreNLP':
                if all_analyses.get() == 'Clause analysis':
                    mb.showwarning(title='Input file error',
                                   message='The CLAUSE analysis algorithm requires a CoNLL table generated by the Stanford CoreNLP PCFG parser.\n\nClause tags are not available in ' + source_package + ' CoNLL tables.\n\nSkipping clause analysis.')
                    print('Clause analysis skipped: requires CoreNLP PCFG parser (source: ' + source_package + ').')
                else:
                    print('Clause analysis skipped for ' + source_package + ' CoNLL table (no Clause Tag column).')
            else:
