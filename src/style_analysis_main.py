...
def run():
    ...
    complexity_readability_analysis_var = globals()['complexity_readability_analysis_var'].get()
    complexity_readability_analysis_menu_var = globals()['complexity_readability_analysis_menu_var'].get()
    ...
    gender_guesser_var = globals()['gender_guesser_var'].get()
...
def help_buttons(window,help_button_x_coordinate,y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(window,help_button_x_coordinate,y_multiplier_integer,"NLP Suite Help",
                                                         'Please, tick the \'Complexity/readability analysis\' checkbox if you wish to analyze the complexity or readability of sentences and documents.\n\nUse the dropdown menu to select the type of analysis to run.\n\n   1. Sentence complexity to provide different measures of sentence complexity: Yngve Depth, Frazier Depth, and Frazier Sum. These measures are closely associated to the sentence clause structure. The Frazier and Yngve scores are very similar, with one key difference: while the Frazier score measures the depth of a syntactic tree, the Yngve score measures the breadth of the tree.\n\nTHE SENTENCE COMPLEXITY ALGORITHM IS BASED ON STANZA.\n\n   2. Text readability to compute various measures of text readability.\n 12 readability score requires HIGHSCHOOL education;\n 16 readability score requires COLLEGE education;\n 18 readability score requires MASTER education;\n 24 readability score requires DOCTORAL education;\n >24 readability score requires POSTDOC education.\n\nTHE TEXT READABILITY ALGORITHM IS BASED ON TEXTSTAT.\n\nWHY READABILITY SCORES CAN BE NEGATIVE OR EXTREME\n\nThe readability formulas (Flesch Reading Ease, Flesch-Kincaid Grade, Gunning Fog, Coleman-Liau, ARI) were calibrated in the 1940s-1970s for typical English prose. Edge cases can produce unexpected values:\n\n• Flesch Reading Ease uses the formula 206.835 - 1.015*(words/sentences) - 84.6*(syllables/words). Very long sentences with many polysyllabic words push the score below zero. Academic and legal texts routinely produce low or negative raw scores.\n\n• Flesch-Kincaid Grade can go negative for extremely short, simple texts (e.g., one-word sentences like "Stop. Go. Run.").\n\n• Gunning Fog penalizes polysyllabic words (3+ syllables). Scientific writing with technical vocabulary (e.g., "electromagnetic", "photosynthesis") produces very high Fog scores, even when the prose is otherwise clear.\n\nThe NLP Suite clamps all scores to sensible ranges (Flesch Reading Ease: 0-121; grade-level indices: 0-30). A score of 0 may indicate the raw formula produced a negative value that was clamped.\n\nReadability scores are most useful for COMPARING documents within a corpus, not for absolute judgments. Look at the range, trends, and outliers across your documents.')
    y_multiplier_integer = GUI_IO_util.place_help_button(window,help_button_x_coordinate,y_multiplier_integer,"NLP Suite Help",
