...
if IO_libraries_util.install_all_packages(GUI_util.window,"search_byWord_main.py",['os','tkinter'])==False:
    sys.exit(0)
...
def run(inputFilename,input_main_dir_path, output_dir_path,
    ...
    # TODO need to substitute with the search util
    if IO_libraries_util.inputProgramFileCheck('file_splitter_util.py')==False:
        return
    ...
    for file in files:
        ...
        if search_by_keyword_var:
            ...
            if file[-4:]=='.txt':
                import search_byWord_util
                search_byWord_util.run(file, output_dir_path, keyword_value_var, lemmatize_var)
