...
def NER_tags_BERT(window, inputFilename, inputDir, outputDir, mode, createCharts, chartPackage):
    ...
    inputDocs = IO_files_util.getFileList(window, inputFilename, inputDir, fileType='.txt')
    ...
    for doc in inputDocs:
        ...
        header = ["Word ID", "Word", "Tag", "Sentence ID", "Sentence", "Document ID", "Document"]
        ...
        for s in sentences:
            ...
            for el in ner_result:
                result.append([el['index'], el['word'], el['entity'], sentenceID, s, documentID,
...
def doc_summary_BERT(window, inputFilename, inputDir, outputDir, mode, createCharts, chartPackage):
    ...
    for doc in inputDocs:
        ...
        inputs = tokenizer([fullText], padding="max_length", truncation=True, max_length=512, return_tensors="pt")
        input_ids = inputs.input_ids.to(device)
        attention_mask = inputs.attention_mask.to(device)

        output = model.generate(input_ids, attention_mask=attention_mask)

        result_output = tokenizer.decode(output[0], skip_special_tokens=True)

        result_summary_list.append(
            [inputFilename, result_output, documentID, IO_csv_util.dressFilenameForCSVHyperlink(doc)])
...
# did not find a bert model on huggingface, instead found one on spacy.io
# spell checks every sentence per doc, and if there is an mistake to correct, it creates a new sentence with the correct spelling and stores both old and new sentences in a csv file
def spell_checker_bert(window, inputFilename, inputDir, outputDir, mode, createCharts, chartPackage):
    result = []
    nlp = spacy.load('en_core_web_sm')
    contextualSpellCheck.add_to_pipe(nlp)
    inputDocs = IO_files_util.getFileList(inputFilename, inputDir, fileType='.txt')
    header = ["Sentence ID", "Sentence", "Spell-Checked Sentence", "Document ID", "Document"]
    Ndocs = str(len(inputDocs))
    ...
    for doc in inputDocs:
        ...
        for s in sentences:
            ...
            if doc._.performed_spellCheck is True:
                result.append([sentenceID, s, checked_text._.outcome_spellCheck, documentID,
                               IO_csv_util.dressFilenameForCSVHyperlink(doc)])
...
def word_embeddings_BERT(window, inputFilename, inputDir, outputDir, openOutputFiles, createCharts, chartPackage):
...
def spell_checker_bert(window, inputFilename, inputDir, outputDir, mode, createCharts, chartPackage):
    ...
    for doc in inputDocs:
        ...
        for s in sentences:
            ...
            if checked_text._.performed_spellCheck is True:
                result.append([sentenceID, s, checked_text._.outcome_spellCheck, documentID,
                               IO_csv_util.dressFilenameForCSVHyperlink(doc)])
