...
def run(inputFilename, inputDir, outputDir,
    ...
    if corpus_statistics_var and ('Word frequency' in corpus_statistics_options_menu_var or '*' in corpus_statistics_options_menu_var):
...
extra_GUIs_menu_var.set('')
extra_GUIs_menu = tk.OptionMenu(window,extra_GUIs_menu_var,'N-grams & Co-occurrences', 'CoNLL Table Analyzer (Open GUI)','Style Analysis (Open GUI)','What\'s in Your Corpus (Open GUI)')
...
TIPS_lookup = {'Style analysis':'TIPS_NLP_Style analysis.pdf',
               ...
               'TF-IDF, Lexical diversity, Word frequency': 'TIPS_NLP_Corpus text analysis tools.pdf'}

TIPS_options='Style analysis', 'English Language Benchmarks', 'N-Grams (word & character)','NLP Ngram and Word Co-Occurrence VIEWER','Google Ngram Viewer','Excel smoothing data series', 'csv files - Problems & solutions', 'Statistical measures', 'Text readability', 'TF-IDF, Lexical diversity, Word frequency'
...
def help_buttons(window,help_button_x_coordinate,y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(window, help_button_x_coordinate, y_multiplier_integer, "NLP Suite Help",
                                'Please, tick the checkbox if you wish to compute basic statistics on your corpus.\n\nUse the dropdown menu to select the desired option. Use the Text options dropdown menu to lemmatize words or exclude stopwords/punctuation.\n\nIn INPUT the script expects a single txt file or a directory containing a set of txt files.\n\nIn OUTPUT, the script generates a set of csv files each containing word n-grams between 1 and 3.\n\nWhen n-grams are computed by sentence index, the sentence displayed in output is always the first occurring sentence.\n\nREADABILITY SCORES\n\nThe readability formulas (Flesch Reading Ease, Flesch-Kincaid Grade, Gunning Fog, Coleman-Liau, ARI) were calibrated in the 1940s-1970s for typical English prose. Edge cases can produce unexpected values:\n\n• Flesch Reading Ease uses 206.835 - 1.015*(words/sentences) - 84.6*(syllables/words). Very long sentences with many polysyllabic words push the score below zero. Academic and legal texts routinely produce low or negative raw scores.\n\n• Flesch-Kincaid Grade can go negative for extremely short, simple texts (e.g., one-word sentences).\n\n• Gunning Fog penalizes polysyllabic words (3+ syllables); scientific writing with technical vocabulary produces very high scores.\n\nThe NLP Suite clamps all scores to sensible ranges (Flesch Reading Ease: 0-121; grade-level indices: 0-30). If you see a score of 0 it may indicate the raw formula produced a negative value that was clamped.\n\nTF-IDF (Term Frequency-Inverse Document Frequency) identifies the most distinctive words per document — words that are frequent in one document but rare across the corpus.\n\nLEXICAL DIVERSITY measures vocabulary richness: TTR (type-token ratio), Root TTR (Guiraud), Log TTR (Herdan), MTLD (sequential TTR factoring), and vocd-D (random-sampling curve fitting).\n\nWORD FREQUENCY DISTRIBUTION plots rank vs. frequency on a log-log scale to test Zipf\'s Law compliance, and shows how many unique words cover 50% and 90% of all tokens.')
    y_multiplier_integer = GUI_IO_util.place_help_button(window, help_button_x_coordinate, y_multiplier_integer, "NLP Suite Help",
