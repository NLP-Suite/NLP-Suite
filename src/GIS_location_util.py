...
sufs = ''
multi_word_location_prefix_table = pd.read_csv(os.path.join(GUI_IO_util.wordLists_libPath,"multi_word_locations.csv"))

# get comma-separated string of multi_word_location prefixes
for p in multi_word_location_prefix_table["Prefix"]:
	...
	if pd.notna(p):
		multi_word_location_prefix += p + ", "
for s in multi_word_location_prefix_table["Suffix"]:
	if pd.notna(s):
		sufs += s + ", "
# trim off the trailing comma and space characters
multi_word_location_prefix=multi_word_location_prefix[:-2]

# convert string to list; no longer used
# multi_word_location_prefix = multi_word_location_prefix.split(', ')
