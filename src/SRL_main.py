...
extra_GUIs_checkbox = GUI_theme_util.create_checkbox(window, text='GUIs available for more analyses ', variable=extra_GUIs_var, onvalue=1, offvalue=0, command=lambda: open_GUI())
...
extra_GUIs_menu = GUI_theme_util.create_option_menu(window, variable=extra_GUIs_menu_var, values=['Subject-Verb-Object (SVO)','Semantic analysis (Open GUI)','Parsers & annotators (Open GUI)','N-grams & Co-Occurrences (Open GUI)','CoNLL table analyzer (Open GUI)','WordNet (Open GUI)','What\'s in Your Corpus (Open GUI)'])
...
SRL_var.set(1)
SRL_checkbox = GUI_theme_util.create_checkbox(window, variable=SRL_var, onvalue=1, offvalue=0, text="Semantic Role Labelling (SRL)")
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(window, GUI_IO_util.labels_x_coordinate,
                                               y_multiplier_integer,
                                               SRL_checkbox, False, False, False, False, 90,
                                               GUI_IO_util.labels_x_coordinate,
                                               "Tick to run Semantic Role Labeling (SRL): for every verb in a sentence, identify WHO did WHAT to WHOM - the Agent (ARG0), Patient (ARG1), Recipient (ARG2), and Where/When/How/Why modifiers - enriched with VerbNet classes and FrameNet frames. English only; runs in the isolated Python 3.8 SRL engine.")
...
def help_buttons(window,help_button_x_coordinate,y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(window,help_button_x_coordinate,y_multiplier_integer,"NLP Suite Help","Tick the SRL (Semantic Role Labeling) checkbox to identify, for every verb (predicate) in a sentence, WHO did WHAT to WHOM:\n"
                                  ...
                                  "The first run loads the BERT-based model and may take 30-60 seconds just to START; then SRL processes your corpus with NO progress shown - its isolated engine's output is not streamed back to this window - and on a large corpus this can take a LONG time (hours). The GUI will appear frozen (Not Responding) the whole time. This is normal, NOT a freeze; please be patient and let it finish."+GUI_IO_util.msg_Esc)
...
readMe_message="This tool runs Semantic Role Labeling (SRL): for every verb (predicate) in a sentence it identifies WHO did WHAT to WHOM - the Agent (ARG0, the doer), the Patient (ARG1, the one acted upon/affected), the Recipient or Beneficiary (ARG2), plus modifiers for Where (ARGM-LOC), When (ARGM-TMP), How (ARGM-MNR), and Why (ARGM-CAU).\n\nSRL is the richer successor to Subject-Verb-Object (SVO) analysis. In INPUT it expects a txt file or a directory of txt files (ENGLISH ONLY). In OUTPUT it produces a csv file with one row per sentence-and-predicate (a sentence with several verbs yields several rows).\n\nBeyond the raw PropBank arguments, SRL enriches each predicate via SemLink (Palmer's PropBank-VerbNet-FrameNet linking):\n   - Refined roles: VerbNet thematic roles (Agent, Patient/Theme, Experiencer, Stimulus, Recipient, Goal, Result...);\n   - VerbNet class: the sense-disambiguated VerbNet class of the predicate (e.g., murder.01 = murder-42.1) - a backbone for grouping verbs into categories such as 'violence';\n   - FrameNet frame: the disambiguated FrameNet frame (Killing, Destroying, Execution, Attack, Cause_harm...) - interpretable action categories for content analysis (e.g., lynch = Killing).\n\nVisualizations include a 'who did what to whom' network and Sankey flow (both entity-level and VerbNet-role-level), plus frequency charts of the refined roles, VerbNet classes, and FrameNet frames.\n\nSRL runs in a separate, isolated Python 3.8 engine (it cannot share the Suite's packages) that is set up once per machine by running  python setup_SRL.py  - this creates the environment, downloads the BERT model, and fetches the SemLink maps. The first run loads the BERT-based model and may take 30-60 seconds just to START; then SRL processes your corpus with NO progress shown - its isolated engine's output is not streamed back to this window - and on a large corpus this can take a LONG time (hours). The GUI will appear frozen (Not Responding) the whole time. This is normal, NOT a freeze; please be patient and let it finish."
