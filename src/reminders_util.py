...
message_TensorFlow = 'The NLP packages BERT and Stanza used in the NLP Suite rely on the Google Machine Learning platform TensorFlow.\nOn a Mac with the new M1 or M2 chip TensorFlow may lead to various errors displayed in terminal (e.g., semaphore) and an inability to run any of the scripts in the NLP Suite.\n\nIf you do experience this problem, while waiting for a more permanent solution to the problem, follow the instructions provided here by Apple\n\nhttps://developer.apple.com/metal/tensorflow-plugin/'
