...
def getDictFile():
    ...
    if ancestor_bySentenceID_var.get() == False:
        ...
        filePath = tk.filedialog.askopenfilename(title='Select INPUT csv file', initialdir=initialFolder,
...
ancestor_var.set(0)
ancestor_checkbox = tk.Checkbutton(window, text='Zoom OUT/UP to aggregate lemmatized words', variable=ancestor_var,
                                   onvalue=1, offvalue=0)
y_multiplier_integer = GUI_IO_util.placeWidget(GUI_IO_util.get_labels_x_coordinate(), y_multiplier_integer,
                                               ancestor_checkbox)
...
dict_WordNet_filename_lb = tk.Label(window, text='csv file of WordNet aggregated words (from ZOOM OUT/UP)')
y_multiplier_integer = GUI_IO_util.placeWidget(GUI_IO_util.get_entry_box_x_coordinate(), y_multiplier_integer,
    ...
    if character_var.get() == True:
        ...
        if fromAncestor:
            mb.showwarning(title='csv WordNet dictionary',
                           message="This is a reminder that you are searching WordNet for " + noun_verb_menu_var.get() + ".\n\nPlease, use the IO widget 'Select INPUT csv file' at the top of the GUI to select the csv file containing LEMMATIZED " + noun_verb_menu_var.get() + " values to be aggregated.\n\nThis file MUST contain LEMMATIZED " + noun_verb_menu_var.get() + " values. If this is not the case, select a different NOUN/VERB option, and/or a different input file option.")
        if GUI_util.inputFilename.get() == '':
            mb.showwarning(title='Warning',
                           message='The Zoom OUT/UP option requires in input a csv file with the LEMMATIZED words (noun or verb) for which you need to find their aggregate (e.g., the verb walk as motion).\n\nRemember to select the INPUT csv file in the IO widgets above.')
        GUI_util.select_input_file_button.configure(state="normal")
...
def activate_CoNLL(*args):
    if extract_nouns_verbs_var.get() == True:
        ...
        mb.showwarning("csv CoNLL table",
                       "Please, use the IO widget 'Select INPUT csv file' at the top of the GUI to select the CoNLL table from which you want to extract nouns and verbs then click on the RUN button.")
    else:
        GUI_util.select_input_file_button.configure(state="disabled")
...
def activate_WordNet(*args):
    if extract_proper_nouns_var.get() == True or extract_improper_nouns_var.get() == True:
        ...
        mb.showwarning("csv WordNet dictionary",
                       "Please, use the IO widget 'Select INPUT csv file' at the top of the GUI to select the WordNet dictionary file generated by the ZOOM IN/DOWN function then click on the RUN button.")
    else:
        GUI_util.select_input_file_button.configure(state="disabled")
