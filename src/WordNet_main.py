    ...
    if aggregate_lemmatized_var==True:
        ...
        if noun_verb=='VERB' and 'verbs_lemma' not in csv_file:
            if hidden_verb_lemma_csv.get()!='':
                ...
                mb.showerror(title='Missing required information', message="You have selected to run the option 'Zoom OUT/UP to find higher-level aggregates' with the 'VERB' option but the csv file currently selected does not contain the expected subscript 'verbs_lemma'.\n\nIf this an overshigth, click on the Select INPUT CSV file button to select a different csv file and try again.")
...
csv_file_button=tk.Button(window, width=GUI_IO_util.select_file_directory_button_width, text='Select INPUT CSV file',command=lambda: get_dictionary_file(window,'Select INPUT csv file', [("dictionary files", "*.csv")],True))
...
def activate_allOptions(noun_verb, fromaggregate=False):
    ...
    if annotate_file_var.get() == 1:
        if csv_file_var.get()=='' and asked==False:
            ...
            filePath=get_dictionary_file(window, 'Select INPUT csv dictionary file', [("dictionary files", "*.csv")], True)
            if filePath=='':
                return
    ...
    if extract_proper_nouns_var.get() == True or extract_improper_nouns_var.get() == True:
        ...
        if csv_file_var.get() == '' and asked==False:
            ...
            filePath = get_dictionary_file(window, 'Select INPUT csv dictionary file', [("dictionary files", "*.csv")],
                                           False)
            if filePath == '':
                return
    # else:
    ...
    if aggregate_lemmatized_var.get() == 1: # aggregating UP
        if fromaggregate==False:
            if csv_file_var.get() == '' and asked==False:
                ...
                filePath =get_dictionary_file(window, 'Select INPUT csv dictionary file', [("dictionary files", "*.csv")], False)
                if filePath == '':
                    return
        csv_file_button.config(state='normal')
    ...
    if extract_nouns_verbs_from_CoNLL_var.get()==True:
        ...
        if csv_file_var.get() == '' and asked==False:
            ...
            filePath = get_dictionary_file(window, 'Select INPUT csv CoNLL file', [("CoNLL files", "*.csv")],
                                           False)
            if filePath == '':
                return
    # else:
    ...
    if aggregate_bySentenceID_var.get() == True:
        ...
        if csv_file_var.get() == '' and asked==False:
            ...
            filePath = get_dictionary_file(window, 'Select INPUT csv file of words classified/aggregated into WordNet synsets', [("csv files", "*.csv")],
                                           False)
            if filePath == '':
                return
            mb.showwarning("csv WordNet dictionary",
