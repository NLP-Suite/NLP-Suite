...
def Wordnet_bySentenceID(ConnlTable, wordnetDict,outputFilename,outputDir,noun_verb,openOutputFiles,createExcelCharts):
    ...
    connl = connl[['word','lemma','postag','SentenceID','DocumentID','Document']]
    ...
    connl = connl[['word','lemma','postag','Category','SentenceID','DocumentID','Document']]
    # put headers on conll list
    connl.columns = ['word','lemma','postag','Category','SentenceID','DocumentID','Document']
    ...
    df = pd.DataFrame(Row_list,index=['word','lemma','postag','WordNet Category','SentenceID','DocumentID','Document'])
    ...
    if createExcelCharts:
        outputFiles=Excel_util.compute_csv_column_frequencies(GUI_util.window,
                                    ...
                                    createExcelCharts,
                                    [[4,5]],
                                    ['WordNet Category'],['word'], ['DocumentID','SentenceID','Document'],
                                    'WordNet', 'line')
...
def character_GoingDOWN(WordNetDir,outputDir, wordNet_keyword_list, noun_verb):
    filesToOpen=[]
...
def ancestor_GoingUP(WordNetDir, inputFile, outputDir, noun_verb,openOutputFiles,createExcelCharts):
