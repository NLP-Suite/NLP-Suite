...
if IO_libraries_util.install_all_Python_packages(GUI_util.window, "GIS_KML_util",
		...
		for row in inputfile:
			...
			if row[3] != 0 and row[2] != 0:
				...
				pnt = GIS_Google_pin_util.pin_customizer(inputFilename, pnt, index, index_list,
														 ...
														 group_var=0, group_number_var=1,
			...
			for row in inputfile:
				for b in range(len(values_row_num)):
					if index - 1 == values_row_num[b]:
						...
						pnt.style.iconstyle.icon.href = icon_url
						sentence=''
						pnt = GIS_Google_pin_util.pin_customizer(inputFilename, pnt,
											 index, index_list,
											 locationColumnName, sentence,
