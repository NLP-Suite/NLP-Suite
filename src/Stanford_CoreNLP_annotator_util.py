    ...
    for j in range(len(filesToVisualize)):
        if 'gender' in str(filesToVisualize[j]):
            ...
            filesToOpen=visualize_Excel_chart(createExcelCharts, filesToVisualize[j], outputDir, filesToOpen, [[3, 3]], 'bar',
                                  'Frequency Distribution of Tenses of Normalized Dates', 1, [], 'NER_tense_bar')
            filesToOpen=visualize_Excel_chart(createExcelCharts, filesToVisualize[j], outputDir, filesToOpen, [[4, 4]], 'bar',
                                              'Frequency Distribution of Information of Normalized Dates', 1, [], 'NER_info_bar')
            # filesToOpen.append(visualize_date_distribution(filesToVisualize[j]))
    p.kill()
...
def visualize_Excel_chart(createExcelCharts,inputFilename,outputDir,filesToOpen, columns_to_be_plotted, chartType, chartTitle, count_var, hover_label, outputFileNameType):
