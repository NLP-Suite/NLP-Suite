...
if IO_libraries_util.install_all_Python_packages(GUI_util.window,"knowledge_graphs_main.py",['os','tkinter','subprocess'])==False:
    sys.exit(0)
...
def run():
    ...
    knowledge_graphs_var = globals()['knowledge_graphs_var'].get()
    confidence_level = confidence_level_entry.get()
    databases_var = globals()['databases_var'].get()
    sub_class_entry = sub_class_entry_var.get()
    ontology_list = globals()['ontology_list']
    create_HTML_files_var = globals()['create_HTML_files_var'].get()
    ontology_color_list = globals()['ontology_color_list']
    bold_DBpedia_YAGO_var = globals()['bold_DBpedia_YAGO_var'].get()
...
def help_buttons(window,help_button_x_coordinate,y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(window,help_button_x_coordinate,y_multiplier_integer,"NLP Suite Help", 'Once you select DBpedia or YAGO, the "Ontology sub-class" widget will become available.\n\nSince the ontology class dropdown menu only includes the main classes in the DBpedia or YAGO ontology, enter in the "Ontology sub-class" widget more specific, comma-separated, sub-classes. You obtain these sub-classes from the TIPS_NLP_Annotator DBpedia ontology classes.pdf or TIPS_NLP_YAGO (schema.org).')
