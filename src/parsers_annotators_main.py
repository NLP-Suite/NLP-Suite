...
extra_GUIs_checkbox = GUI_theme_util.create_checkbox(window, text='GUIs available for pre-processing ', variable=extra_GUIs_var, onvalue=1, offvalue=0, command=lambda: open_GUI(extra_GUIs_menu_var.get()))
...
extra_GUIs_menu = GUI_theme_util.create_option_menu(window,variable=extra_GUIs_menu_var,values=['File checking & cleaning (Open GUI)','Coreference resolution (Open GUI)'])
...
parser_checkbox = GUI_theme_util.create_checkbox(window, variable=parser_var, onvalue=1, offvalue=0, command=lambda: activate_NLP_options())
# place widget with hover-over info
...
if len(parsers) == 0:
    parser_menu = GUI_theme_util.create_option_menu(window, variable=parser_menu_var, values=[])
    # parser_menu.configure(width=50)
else:
    parser_menu = GUI_theme_util.create_option_menu(window, variable=parser_menu_var, values=list(parsers))
    # parser_menu.configure(width=50)
#     # place widget with hover-over info
...
Json_checkbox = GUI_theme_util.create_checkbox(window, text='Export Json file',
                               variable=Json_var,
...
CoNLL_table_analyzer_checkbox = GUI_theme_util.create_checkbox(window, text='CoNLL table analyzer', variable=CoNLL_table_analyzer_var,
                                               onvalue=1, offvalue=0, command=lambda: check_CoNLL_table())
# place widget with hover-over info
y_multiplier_integer = GUI_IO_util.placeWidget(window, GUI_IO_util.labels_x_indented_coordinate, y_multiplier_integer,
                                               CoNLL_table_analyzer_checkbox, True, False, False, False, 90,
                                               GUI_IO_util.labels_x_coordinate,
                                               "The CoNLL table analyzer can be used with CoNLL tables generated by any parser (spaCy, Stanford CoreNLP, Stanza)")

CoNLL_table_analyzer_checkbox_msg = GUI_theme_util.create_label(window)
CoNLL_table_analyzer_checkbox_msg.configure(text="Open the CoNLL table analyzer GUI")
...
def check_CoNLL_table(*args):
    if CoNLL_table_analyzer_var.get() == 1:
        CoNLL_table_analyzer_checkbox_msg.configure(text="Open CoNLL table analyzer GUI")
    else:
        CoNLL_table_analyzer_checkbox_msg.configure(text="Do NOT open CoNLL table analyzer GUI")
...
Annotators_checkbox = GUI_theme_util.create_checkbox(window, text='Annotators', variable=annotators_var,
                                             onvalue=1, offvalue=0)
# place widget with hover-over info
...
annotators_menu_var.set("")
annotators_menu_var.set("")
annotators_menu = GUI_theme_util.create_option_menu(window, variable=annotators_menu_var, values=[
        'Fundamental NLP tools (via CoreNLP, spaCy, Stanza) -----------------------------------------------',
...
manual_Coref_checkbox = GUI_theme_util.create_checkbox(window, text='Manual edit',
                                       variable=manual_Coref_var,
                                       onvalue=1, offvalue=0)

open_GUI_checkbox = GUI_theme_util.create_checkbox(window, text='Open coreference GUI',
                                       variable=open_GUI_var,
                                       onvalue=1, offvalue=0)

quote_checkbox = GUI_theme_util.create_checkbox(window, text='Include single quotes',
                                       variable=quote_var,
                                       onvalue=1, offvalue=0)
...
def activate_NLP_options(*args):
    ...
    if package_display_area_value_new != package_display_area_value:
        ...
        GUI_theme_util.set_values(parser_menu, [s.lstrip() for s in parsers])

    try:
        parser_lb.configure(text=available_parsers)
    except NameError:
        parser_lb = GUI_theme_util.create_label(window, text=available_parsers)
