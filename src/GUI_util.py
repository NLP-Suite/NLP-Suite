...
def display_logo():
    ...
    try:
        ...
        for x in image_list:
            ...
            # the ? HELP column's left edge, minus the ~12px of built-in whitespace on the logo's left, so
            # the mark sits flush above the ? HELP buttons: the reduced grid margin, or the legacy x for a
            # GUI that opted out of grid and still places ? HELP at help_button_x_coordinate.
...
def GUI_bottom(config_filename, config_input_output_numeric_options, y_multiplier_integer, readMe_command,
    ...
    # An opted-out GUI (GUI_IO_util.GRID_OPT_OUT) is absolutely .place'd -- nothing to finalize, and its
    # window keeps its hand-tuned size. Everything below is grid-only.
    if GUI_IO_util.grid_layout_enabled:
        ...
        #
        # This is the half of the fix that matters on a Mac. Every GUI's size was hand-measured on
        # Windows, where Tk's default font is ~9pt; macOS gives it ~13pt, so the same labels come out
        # roughly 40% wider and the right-hand end of each row falls outside a window that was never told
        # to grow. Grid knows exactly how wide the content is, so ask it and widen the window to match
        # instead of clipping. Never shrink below the hand-tuned size -- GUIs leave room for widgets that
        # only appear once an option is ticked -- and never exceed the screen.
        try:
