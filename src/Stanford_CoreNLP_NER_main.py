...
if IO_libraries_util.install_all_packages(GUI_util.window, "Stanford_CoreNLP_NER_extractor",
...
def run(inputFilename, inputDir, outputDir, openOutputFiles, createExcelCharts, encoding_var,
    ...
    if len(NER_list) == 0:
        mb.showwarning(title='No NER tag selected',
                       message='No NER tag has been selected.\n\nPlease, select an NER tag and try again.')
        return
    ...
    IO_user_interface_util.timed_alert(GUI_util.window, 3000, 'Analysis end', 'Finished running NER extraction at',
                                       True)
...
extract_date_from_filename_checkbox = tk.Checkbutton(window, variable=extract_date_from_filename_var, onvalue=1,
                                                     offvalue=0)
...
tk.Label(window, text='NER tag').place(x=GUI_IO_util.get_labels_x_coordinate(),
...
y_multiplier_integer = GUI_IO_util.placeWidget(GUI_IO_util.get_labels_x_coordinate(), y_multiplier_integer, NER_menu_lb,
                                               True)

NER_menu = tk.OptionMenu(window, NER_tag_var, '--- All NER tags', '--- All quantitative expressions', 'NUMBER',
                         'ORDINAL', 'PERCENT', '--- All social actors', 'PERSON', 'ORGANIZATION',
                         '--- All spatial expressions', 'CITY', 'STATE_OR_PROVINCE', 'COUNTRY',
                         '--- All temporal expressions', 'DATE', 'TIME', 'DURATION', 'SET', '--- All other expressions',
                         'MISC', 'CAUSE_OF_DEATH', 'CRIMINAL_CHARGE', 'EMAIL', 'IDEOLOGY', 'LOCATION', 'MONEY',
                         'NATIONALITY', 'RELIGION', 'TITLE', 'URL')
y_multiplier_integer = GUI_IO_util.placeWidget(GUI_IO_util.get_labels_x_coordinate() + 70, y_multiplier_integer,
                                               NER_menu, True)

add_NER_button = tk.Button(window, text='+', width=2, height=1, state='disabled',
                           command=lambda: activate_NER_Options())
y_multiplier_integer = GUI_IO_util.placeWidget(GUI_IO_util.get_entry_box_x_coordinate() + 40, y_multiplier_integer,
                                               add_NER_button, True)

reset_NER_button = tk.Button(window, text='Reset', width=5, height=1, state='disabled',
                             command=lambda: clear_NER_list())
y_multiplier_integer = GUI_IO_util.placeWidget(GUI_IO_util.get_entry_box_x_coordinate() + 70, y_multiplier_integer,
                                               reset_NER_button, True)

NER_entry_lb = tk.Label(window, text='NER list')
y_multiplier_integer = GUI_IO_util.placeWidget(GUI_IO_util.get_entry_box_x_coordinate() + 140, y_multiplier_integer,
                                               NER_entry_lb, True)
...
def add_NER_tag(*args):
    ...
    if NER_tag_var.get() == '--- All NER tags':
        NER_list = ['PERSON', 'ORGANIZATION', 'MISC', 'MONEY', 'NUMBER', 'ORDINAL',
                    ...
                    'CAUSE_OF_DEATH']
        NER_entry_var.set(
            'PERSON, ORGANIZATION, MISC, MONEY, NUMBER, ORDINAL, PERCENT, DATE, TIME, DURATION, SET, EMAIL, URL, LOCATION, CITY,STATE_OR_PROVINCE, COUNTRY, NATIONALITY, RELIGION, TITLE, IDEOLOGY, CRIMINAL_CHARGE,CAUSE_OF_DEATH')
        ...
        mb.showwarning(title='Warning',
                       message='You cannot select the option --- All other expressions.\n\nPlease, select a different option and try again.')
        NER_tag_var.set('')
        window.focus_force()
        return
    if NER_tag_var.get() in NER_list and not ('---' in NER_tag_var.get()):
        mb.showwarning(title='Warning',
                       message='The NER tag "' + NER_tag_var.get() + '" is already in your selection list: ' + str(
...
y_multiplier_integer = GUI_IO_util.placeWidget(GUI_IO_util.get_labels_x_coordinate(), y_multiplier_integer,
                                               NER_sentence_checkbox)

TIPS_lookup = {'NER (Named Entity Recognition)': 'TIPS_NLP_NER (Named Entity Recognition).pdf',
...
def help_buttons(window, help_button_x_coordinate, basic_y_coordinate, y_step):
    ...
    GUI_IO_util.place_help_button(window, help_button_x_coordinate, basic_y_coordinate + y_step * 4, "Help",
                                  "Please, using the scale widget, adjust the memory size you wish to use. The default size of 4 should be quite sufficient for the Stanford CoreNLP NER annotator." + GUI_IO_util.msg_Esc)
    ...
    GUI_IO_util.place_help_button(window, help_button_x_coordinate, basic_y_coordinate + y_step * 7, "Help",
                                  "Please, tick the checkbox if you wish to plot the extracted NER tags by sentence index." + GUI_IO_util.msg_Esc)
