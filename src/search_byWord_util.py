...
def run(inputFilename,outputPath, keyword, lemmatization):
    ...
    with open(csvtitle, "a",newline = "", encoding='utf-8',errors='ignore') as csvFile:
        ...
        for sent in sentences_: 
            ...
            if kw == True:#if keyword is detected, generate the next subfile
                ...
                if lemmatization: 
                    writer.writerow([docIndex, inputFilename, sentence_index, sent, keyword, form,  first_occurrence_index, sentence_index / len(sentences_), frequency])
                else: 
                    writer.writerow([docIndex, inputFilename, sentence_index, sent,  keyword, '', first_occurrence_index, sentence_index / len(sentences_), frequency])
                # writer.writerow([docIndex, inputFilename, presubfile, keyword, sent, first_occurrence_index, sentence_index / len(sentences_), frequency])
                 ...
                 writer.writerow([docIndex, inputFilename, sentence_index, sent,  '', '', '', '', ''])
            sentence_index += 1
