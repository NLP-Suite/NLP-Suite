...
def run(inputFilename,input_main_dir_path, outputPath, search_by_dictionary_var, search_by_keyword_var, keyword, lemmatization, within_sentence):
    ...
    for file in files:
        ...
        keywordtokens = keyword.split(',') #word_tokenize(keyword)
        ...
        with open(csvtitle, "a",newline = "", encoding='utf-8',errors='ignore') as csvFile:
            ...
            if csvExist == False:
                writer.writerow(["SEARCH_WORD", "LEMMATIZED", "Sentence ID of FIRST_OCCURRENCE", "RELATIVE_POSITION", "FREQUENCY of OCCURRENCE", "Document ID", "Document", "Sentence ID","SENTENCE"])
                docIndex = 1
            else:
            ...
            for sent in sentences_:
                ...
                for keywordtoken in keywordtokens:
                    if keywordtoken in sent:
                        for kindex in keywordtoken.split(' '):
                            for token in tokens_:
                                t = token.lower()
                                if kwindex == len(kwlist):
                                    break
                                if t == kwtokens[kwindex]:
                                    kw = True
                                    kwindex += 1
                                    form += t + " "
                                else:
                    ...
                    if len(sent)>0:
                        if lemmatization:
                            form=keywordtoken #RF
                            writer.writerow([keyword, form,  first_occurrence_index, sentence_index / len(sentences_), frequency, docIndex, IO_csv_util.dressFilenameForCSVHyperlink(file), sentence_index, sent])
                        else:
                            writer.writerow([keyword, '', first_occurrence_index, sentence_index / len(sentences_), frequency, docIndex, IO_csv_util.dressFilenameForCSVHyperlink(file), sentence_index, sent])
                sentence_index += 1
