...
if not IO_libraries_util.install_all_packages(GUI_util.window, "spell-checker_main.py",
                                              ['pycorenlp', 'os', 're', 'stanfordcorenlp', 'nltk', 'pandas',
...
def run(inputFilename, inputDir, outputDir,
    ...
    if spelling_checker_var:
        IO_user_interface_util.timed_alert(GUI_util.window, 3000, 'Spelling checker start',
                                           'Started running Spelling checker at', True,
                                           'You can follow Spelling checker in command line.')

        if checker_value_var == '*' or checker_value_var == "Spell checker (via NLTK unusual words)":
            print("checker_value_var", checker_value_var)
            file_spell_checker_util.nltk_unusual_words(GUI_util.window, inputFilename, inputDir, outputDir, openOutputFiles,
                                                  createExcelCharts)

        # It would be a good idea to separate I/O operation and text processing to make codes cleaner.
        folderID = 0
        fileID = 0
            ...
            for filename in files:
                ...
                with open(input_files_path, 'r', encoding='utf-8', errors='ignore') as opened_file:
                    ...
                    if checker_value_var == '*' or 'autocorrect' in checker_value_var:
                        text, csv = file_spell_checker_util.spellchecking_autocorrect(text)
                        print(text)
                        csv["Filename"] = [filename] * csv.shape[0]
                    ...
                    if checker_value_var == '*' or 'pyspellchecker' in checker_value_var:
                        text, csv = file_spell_checker_util.spellchecking_pyspellchecker(text)
                        csv["Filename"] = [filename] * csv.shape[0]
                    ...
                    if checker_value_var == '*' or 'textblob' in checker_value_var:
                        text, csv = file_spell_checker_util.spellchecking_text_blob(text)
                        csv["Filename"] = [filename] * csv.shape[0]
                        textblob_df = pandas.concat([textblob_df, csv], ignore_index=True)
                        print(text)

                    corrected_files_path = os.path.join(corrected_files_dir, filename)
                    with open(corrected_files_path, 'w+', encoding='utf-8') as file_to_write:
                        file_to_write.write(text)

        if checker_value_var == '*' or 'autocorrect' in checker_value_var:
            autocorrect_df.to_csv(os.path.join(outputDir, 'spell_checker_autocorrect.csv'),index=False)

        if checker_value_var == '*' or 'pyspellchecker' in checker_value_var:
            pyspellchecker_df.to_csv(os.path.join(outputDir, 'spell_checker_pyspellchecker.csv'),index=False)

        if checker_value_var == '*' or 'textblob' in checker_value_var:
            textblob_df.to_csv(os.path.join(outputDir, 'spell_checker_textblob.csv'),index=False)
...
spelling_checker_value = tk.OptionMenu(window, checker_value_var, '*',
                                       ...
                                       'Spell checker (via Java tool)')
y_multiplier_integer=GUI_IO_util.placeWidget(GUI_IO_util.get_entry_box_x_coordinate(), y_multiplier_integer,spelling_checker_value)
