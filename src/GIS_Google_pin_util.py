...
if IO_libraries_util.install_all_packages(GUI_util.window,"GIS_Google_pin_util",['os','simplekml','tkinter'])==False:
	sys.exit(0)
...
def pin_customizer(inputFilename, pnt, geo_index, index_list, locationColumnName,
	...
	if description_var_list[j] == 1:
		if len(description_csv_field_var_list[j]) == 0:
			mb.showwarning(title='No CSV Field Selected for Description for Group No.'+str(j+1), message='The description checkbox is ticked but no csv field was selected for the Group No.'+ str(j+1) +'.\n\nPlease, check your input and try again.')
			sys.exit()
		if group_var == 1:
...
def pin_description(inputFilename,pnt, data, headers, geo_index, index_list, description_location_var_name, group_var, group_values, group_labels, j, name_var, description_csv_field_var, italic_var, bold_var):
	...
	# for LARGE data the routine is very slow since the loop loops through each record
	# data is a list of lists
	# If data were a numpy array or dataframe,there is a simple one-liner that extracts all descriptions:
	# description = data[:, column_num]
	# The “:” here means “all elements in that column”.
	# For this approach, however, data has to be a numpy array or dataframe.
		...
		if name_var == 0:
			if description_location_var_name == description_csv_field_var:
				if italic_var == 1 and bold_var == 1:
					...
					pnt.description = "<b>Location</b>: " + names[index-1] + "<br/><br/><b>Group Label</b>: " + group_label + "<br/><br/><b>Group Value</b>: " + group_value
				elif italic_var == 1:
					pnt.description = "<i>Location</i>: " + names[index-1] + "<br/><br/><i>Group Label</i>: " + group_label + "<br/><br/><i>Group Value</i>: " + group_value 
				else:
