...
def run():
    ...
    check_tools = check_tools_var.get()
    convert_tools = convert_tools_var.get()
    clean_tools = clean_tools_var.get()
    menu_option = globals()['menu_option']
    script_to_run = globals()['script_to_run']
    function_to_run = globals()['function_to_run']
...
pydict["Document converter (pdf --> txt) (SCANNED pdf, via pytesseract OCR)"] = ["file_converter_util.pdf_OCR_converter"]
pydict["Document converter (pdf --> docx) (text & images via pdfminer & pypdf)"] = ["file_converter_util.pdf_to_docx_converter"]
...
convert_menu = tk.OptionMenu(window,convert_tools_var,
                    ...
                    'Document converter (pdf --> txt) (SCANNED pdf, via pytesseract OCR)',
                    'Document converter (pdf --> docx) (text & images via pdfminer & pypdf)',
...
TIPS_lookup = {'File manager':'TIPS_NLP_File manager.pdf','File handling in NLP Suite': "TIPS_NLP_File handling in NLP Suite.pdf",'Filename checker':'TIPS_NLP_Filename checker.pdf','Filename matcher':'TIPS_NLP_Filename matcher.pdf','File classifier (By date)':'TIPS_NLP_File classifier (By date).pdf','File classifier (By NER)':'TIPS_NLP_File classifier (By NER).pdf','File content checker & converter & cleaner':'TIPS_NLP_File checker & converter & cleaner.pdf','pdf converters (including OCR)':'TIPS_NLP_pdf converters.pdf','Text encoding (utf-8)':'TIPS_NLP_Text encoding (utf-8).pdf','Spelling checker':'TIPS_NLP_Spelling checker.pdf','File merger':'TIPS_NLP_File merger.pdf','File splitter':'TIPS_NLP_File splitter.pdf','Newspaper titles (separate titles from articles)':'TIPS_NLP_Newspaper title.pdf'}
TIPS_options= 'File content checker & converter & cleaner','pdf converters (including OCR)','File handling in NLP Suite', 'File manager', 'Filename checker', 'Filename matcher', 'File classifier (By date)','File classifier (By NER)','Text encoding (utf-8)','Spelling checker','File merger','File splitter','Newspaper titles (separate titles from articles)'
...
def help_buttons(window,help_button_x_coordinate,y_multiplier_integer):
    ...
    y_multiplier_integer = GUI_IO_util.place_help_button(window,help_button_x_coordinate,y_multiplier_integer,"NLP Suite Help","Please, using the dropdown menu, select one of the options available for converting the file type:\n\n   csv to txt;\n   docx (NOT doc) to txt;\n   pdf to txt, for an ordinary pdf;\n   pdf to txt, for a SCANNED pdf (OCR);\n   pdf to docx;\n   rtf to txt.\n\nTXT is the only file type the NLP Suite tools can process. Use a 'to txt' converter when you want to analyze your documents.\n\npdf to txt (via the pdfminer package) can also convert column-based pdf files. It extracts the TEXT ONLY: any image embedded in your pdf files is LOST.\n\npdf to txt (via pytesseract OCR) is for SCANNED pdf files, i.e., pdf files that contain no text but only a picture of each page (if you cannot select the text in a pdf reader, the pdf is scanned). OCR reads the letters out of that picture.\n   OCR is SLOW (a few seconds per page) and never perfect: ALWAYS check the output. For an ordinary pdf use the pdfminer option, which is exact and far faster.\n   Unlike every other converter, OCR needs software the NLP Suite installers cannot bundle: the pytesseract and pdf2image Python modules AND the Tesseract and poppler programs. The script checks all four and names whatever is missing, with the command to install it. See the TIPS file 'pdf converters (including OCR)'.\n\npdf to docx (via the pdfminer and pypdf packages) keeps BOTH the text and the images of your pdf files in a single Word document, which is what you want when you need to preserve the whole document rather than analyze its text.\n   The docx output is a REFLOW of the pdf and NOT a photographic copy: you get the original text and images in reading order, in a single column, but NOT the original fonts, columns and page layout. Tables are converted as lines of text and not as Word tables.\n   This option needs the Python module pypdf. If it is not installed in your NLP environment, the script will tell you the pip command to install it.\n\nMAKE SURE TO OCR THE PDF DOCUMENT(S) BEFORE CONVERTING FOR BETTER QUALITY RESULTS. A scanned pdf (i.e., a pdf that is only a picture of a page) contains no text to extract.\n\nIn INPUT, when a directory is selected, all files in a directory and its subdirectories can be converted. The script will ask users whether they want to convert files in subdirectories.\n\nIn OUTPUT, the converted file(s) will be placed in the OUTPUT directory you selected, in the same subdirectory structure as the input file(s)." + GUI_IO_util.msg_Esc)
