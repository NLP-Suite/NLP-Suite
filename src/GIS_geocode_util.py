...
def nominatim_geocode(geolocator,loc,country_bias='',box_tuple='',timeout=4):
	...
	try:
		return geolocator.geocode(loc,language='en',country_codes=country_bias,viewbox=box_tuple, bounded=1, timeout=timeout)
	...
	for item in locations:
		...
		if str(item)!='nan' and str(item)!='':
			...
			if itemToGeocode in distinctGeocodedList:
				...
				if geocoder=='Nominatim':
					location = nominatim_geocode(geolocator,itemToGeocode,country_bias,area)
				else:
