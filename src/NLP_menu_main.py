...
if sys.platform=='darwin':
    ...
    if platform.machine()!='x86_64':
        import subprocess
        # Use the RUNNING interpreter's pip: a bare "pip" is not on PATH in the bundled app, so
        # subprocess.check_output(["pip", ...]) raised an unhandled exception before mainloop() and
        # the whole app closed when the setup dialog was dismissed. Also wrap it so any failure here
        # is never fatal to startup.
        output = ""
        try:
            output = subprocess.check_output([sys.executable, "-m", "pip", "freeze"]).decode("utf-8")
        except Exception as e:
            print("Could not run pip freeze for the TensorFlow check (non-fatal): ", e)
        print("\n\n--------------- output\n\n--------------- ", str(output))
        # A valid TensorFlow install is tensorflow-macos, tensorflow-metal, OR - since TensorFlow
        # 2.13+ ships arm64 wheels - the standard 'tensorflow' package. Warn only if none is present.
        has_tensorflow = ("tensorflow-macos" in output or "tensorflow-metal" in output
                          or "tensorflow==" in output)
        if output and not has_tensorflow:
            mb.showwarning(title='TensorFlow not installed',
                   message='Your Mac uses Apple Silicon (M1/M2/M3 chips).\n\n'
                   'Some NLP Suite algorithms (e.g., the cutting-edge BERT models built on Google '
                   'TensorFlow) need TensorFlow, which is not currently installed.\n\n'
                   'The bundled Python environment already supports Apple Silicon, so no Anaconda '
                   'changes are needed. To enable these algorithms, open Terminal and type:\n\n'
                   'pip install tensorflow\n\n'
                   'then press Enter and restart the NLP Suite.')
