...
Post-CTk-migration, every GUI in src/ is laid out by the shared grid (placeWidget -> columns) --
there is no more "opted-out absolute .place()" tier (that was closed out in Phase 4, see
docs/CustomTkinter-Migration-Plan.md). The page has a table of contents and one section of cards,
each titled with the GUI's own window caption and carrying a PASS/FAIL badge (overflow/overlap
check); flagged GUIs sort to the top.
...
import GUI_util, GUI_IO_util, GUI_theme_util
...
# CTk widgets are composite (a CTkButton is itself a Frame containing a Canvas + Label): walking into
# every widget's children and flattening the result into one global overlap check flags a button's own
# guts as "overlapping" itself. So (1) treat CTk's atomic controls as leaves -- don't recurse into their
# internals -- and (2) only compare *siblings* for overlap (a child is always geometrically inside its
# parent; that is containment, not two widgets colliding). Transparent CTkFrames used purely as
# .pack(in_=...) grouping boxes (e.g. NLP_menu_main's checkbox+button rows) are excluded from the
# comparison pool -- they intentionally sit under their own children with no visible fill.
LEAF_TYPES = (ctk.CTkButton, ctk.CTkCheckBox, ctk.CTkComboBox, ctk.CTkEntry, ctk.CTkLabel,
              ctk.CTkOptionMenu, ctk.CTkProgressBar, ctk.CTkRadioButton, ctk.CTkSegmentedButton,
              ctk.CTkSlider, ctk.CTkSwitch, ctk.CTkTextbox, ctk.CTkCanvas)

def is_transparent_frame(widget):
    try:
        return isinstance(widget, ctk.CTkFrame) and str(widget.cget('fg_color')) == 'transparent'
    except Exception:
        return False
...
def check_siblings(kids):
    global overlaps
    cmp_kids = [k for k in kids if not is_transparent_frame(k[1])]
    S = sorted(cmp_kids, key=lambda kr: (kr[0][0], kr[0][1]))
...
def walk(widget, ox, oy):
    ...
    for ch in widget.winfo_children():
        try:
            ...
            r = (cx, cy, cx + ch.winfo_width(), cy + ch.winfo_height())
            if r[2] <= r[0] or r[3] <= r[1]:
                continue
            kids.append((r, ch, cx, cy))
            all_rects.append(r)
        except Exception:
            pass
    check_siblings([(r, ch) for r, ch, _, _ in kids])
    for r, ch, cx, cy in kids:
        if not isinstance(ch, LEAF_TYPES):
            walk(ch, cx, cy)

walk(w, 0, 0)
max_right = max((r[2] for r in all_rects), default=0)
...
try:
    ...
    pass
print("META overflow=%d overlaps=%d title=%s"
      % (overflow, overlaps, base64.b64encode(title.encode('utf-8')).decode()))
...
def main():
    ...
    print(
        "Building + screenshotting %d GUIs (windows will flash open)...\n" % len(guis)
    )
    cards = []
    for f in guis:
        ...
        try:
            p = subprocess.run(
                [sys.executable, "-c", _WORKER, f, png],
                capture_output=True,
                text=True,
                timeout=300,
                cwd=_SRC,
        ...
        note = (
            ""
            if overflow is None
            else "overflow %d px &middot; overlaps %d" % (overflow, overlaps)
        )
        cards.append(
            dict(
                status=status,
    ...
    flagged = sorted(
        [c for c in cards if c["status"] != "OK"],
        key=lambda c: (order.get(c["status"], 3), c["file"]),
    )
    ok = sorted(
        [c for c in cards if c["status"] == "OK"],
        key=lambda c: c["file"],
    ...
    def toc(group):
        return (
            "\n".join(
                ...
                % (anchor(c["file"]), c["status"].lower().rstrip("?"), esc(c["title"]))
                for c in group
            )
    ...
    def card_html(c, big):
        if c["png"]:
            ...
            img = (
                '<a href="%s" target="_blank"><img loading="lazy" src="%s" alt="%s"></a>'
    ...
    def section(title, subtitle, group, big=False):
        ...
        return (
            '<h2>%s <span class="cnt">%d</span></h2><p class="sub">%s</p><div class="%s">%s</div>'
            % (
                title,
                len(group),
                subtitle,
...
<p class="sub">"""
        + "%d GUIs &middot; %d flagged &middot; this machine's Windows rendering only &middot; click any shot for full resolution"
        % (len(cards), n_off)
        + """</p>
<div class="toc">
 <div><h3>Flagged</h3><ul>"""
        + toc(flagged)
        + """</ul></div>
 <div><h3>OK ("""
        + str(len(ok))
...
"""
        + section(
            ...
            "Shown large. Overflow &gt; 4px = a widget past the right edge; overlaps = two widgets on one spot. (This machine is 150%-scaled, so wide GUIs clip here that would fit a normal display.)",
            flagged,
            big=True,
        )
        + section(
            "GUIs &mdash; OK",
            "Standard layout via the shared grid; these fit cleanly.",
            ok,
        )
    )
    ...
    with open(out_path, "w", encoding="utf-8") as fh:
        fh.write(html)
    print(
        "\n%d GUIs (%d OK), %d flagged.  Open: %s"
        % (len(cards), len(ok), n_off, out_path)
    )
    return 0
